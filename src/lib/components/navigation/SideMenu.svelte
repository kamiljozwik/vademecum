  ...
  >
    ...
    {#each chaptersWithLessons as chapterData, i}
      ...
      <li class="mr-5 mt-4 flex border-b-2 border-neutral-800 px-1 py-2">
        <a href={`/kurs/${$page.params.course}/${chapterData[0]}`}
          ><h3 class="font-bold uppercase text-neutral-300">
            <span class="gradient-text mr-2">{i + 1}</span><span
              >{chapterData[0]}</span
            >
          </h3></a
        >
      ...
      {#each chapterData[1].lessons as { lessonMeta }}
        <li>
          <a
            ...
            href={`/kurs/${$page.params.course}/${chapterData[0]}/${lessonMeta.slug}`}
          >
        ...
        <details open={isChapterActive(chapterData[0])}>
          <summary
            class={cn("pl-0 text-lg font-bold uppercase text-base-content", {
              "text-base-content dark:text-gray-200": isChapterActive(
                chapterData[0]
              ),
            })}
          ...
          <ul>
            {#each chapterData[1].lessons as { lessonMeta }}
              <li
                class={cn({
                  "text-base-content dark:text-gray-200": isLessonActive(
                    lessonMeta,
                    chapterData[0]
                  ),
                })}
