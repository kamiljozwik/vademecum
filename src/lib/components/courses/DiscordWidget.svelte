<script lang="ts">
	...
	onMount(async () => {
		...
		let res = await fetch(`https://discord.com/api/guilds/${discordApi}/widget.json`);
		data = await res.json();
	});
</script>
	...
	<section class="my-12 flex flex-col items-center justify-between">
		...
		{#if data}
			<div class="mt-2">
				...
				<span class="font-display text-lg text-green-500">{data.presence_count}</span>
				<span class="text-gray-200">osoby online</span>
			</div>
		{/if}
	</section>
{/if}
