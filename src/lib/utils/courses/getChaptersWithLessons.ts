...
export const getChaptersWithLessons = (
	...
	const categoriesWithLessons = new Map<
		string,
		{ lessons: LessonData[]; chapterMeta: Chapter; chapterContent: MarkdownContent }
	>();
	...
	for (const chapter of chapters) {
		...
		const lessons: LessonData[] = Object.entries(lessonsPaths)
			...
			.map((p) => ({
				lessonMeta: { ...p[1].metadata, slug: p[0].split('/').at(-1)?.replace('.md', '') || '' },
		...
		categoriesWithLessons.set(chapter, { lessons, chapterMeta: metadata, chapterContent: content });
	}

	const sortedByChapters = new Map(
		[...categoriesWithLessons].sort((a, b) => a[1].chapterMeta.order - b[1].chapterMeta.order)
	);
