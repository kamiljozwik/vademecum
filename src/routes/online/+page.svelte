	...
	<ul class="flex flex-col-reverse gap-12">
		{#each torender as course}
			...
			<li class="list-none p-0 transition-transform hover:scale-[101%]">
				<article>
					<a href={`online/${course.meta.slug}`} class="no-underline">
						<div class="card bg-base-300 shadow-xl lg:card-side">
							<figure class="lg:w-1/2">
								<img
									class="w-[500px]"
									src={`/online/${course.meta.slug}/img/featured.webp`}
									alt="Okładka kursu"
								/>
							</figure>
							<div class="card-body bg-neutral lg:w-1/2">
								...
								<div class="card-actions">
									<button class="btn btn-accent">{course.meta.cta || 'Zobacz więcej'}</button>
								</div>
							</div>
						</div>
					</a>
